...
import PwdUtil from "@src/util/crypto";
...
describe("AuthRouter", () => {
  ...
  describe(`"POST:${Login}"`, () => {
    ...
    it(`should return a response with a status of "${UNAUTHORIZED}" and a ` + `json with an error message of "${EMAIL_NOT_FOUND_ERR}" if the email ` + "was not found.", (done) => {
      // Spy
    ...
    // Password failed
    it(`should return a response with a status of "${UNAUTHORIZED}" and a ` + `json with the error "${Errors.Unauth}" if the password failed.`, (done) => {
      // Setup data
      const role = UserRoles.Standard,
